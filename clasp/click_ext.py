...
def main(arg1, **kwargs):
    ...
    * parse_file_list: checks if space seperated or wildcard are all files
      (no prompt on error)
    * is_file: check if a single path exists (prompts)
    * are_files: calls parse_file_list with prompt on error
    * are_files_iter: calls are files on a list of strings
    * color_inp: number tuple or string.
    ...
    * split_str: uses shlex to split string
    * tup_list: casts tuple as list
